...
- [x] Vite + React + TailwindCSS project init
...
- [ ] Skill groups display (grouped by category and clusters)
- [ ] Work experience form (title, company, dates, tasks as bullets, tools)
- [ ] Education form
- [ ] Projects form (includes the not_in_project field, which is required for prompt accuracy)
- [ ] Volunteering and interests forms
- [ ] Profile save to Supabase

## Phase 3: Generation

- [ ] Job offer input (paste raw text, with an optional URL)
- [ ] Job analysis (call Claude to extract a structured analysis, save to DB)
...
- [ ] PDF export (react-pdf, using the user's chosen CV template)
- [ ] Applications log (list of all jobs applied to, with status and date)
- [ ] Status tracking (draft, applied, interviewing, rejected, or offered)
- [ ] Link generated documents from the application log
...
- [ ] Demo mode (try the tool without creating an account)

## Ideas for later

- Import a LinkedIn profile to pre-fill the profile builder
